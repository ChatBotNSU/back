...
class Subgraph(BaseModel):
    name: str
    # Input parameter names. The caller MUST bind every one of these to a caller variable.
    # Bindings are by-reference: any modification of an input name inside the subgraph
    # is written back to the bound caller variable on return. Everything else is local.
    inputs: list[str] = []
