"""Unit tests for preview-execution controller/redis_streams.py."""
...
def controller(mock_config, mock_redis):
    ...
    with patch('controller.redis_streams.get_config', return_value=mock_config):
        with patch('controller.redis_streams.get_redis', return_value=mock_redis):
            with patch('controller.redis_streams.PreviewResponseSender') as mock_sender:
...
class TestRedisStreamsControllerInit:
    ...
    def test_initialization(self, mock_config, mock_redis):
        ...
        with patch('controller.redis_streams.get_config', return_value=mock_config):
            with patch('controller.redis_streams.get_redis', return_value=mock_redis):
                with patch('controller.redis_streams.PreviewResponseSender'):
                    # When
                    ctrl = RedisStreamsController()
...
class TestGetExecutionId:
    """Tests for get_execution_id method."""

    @pytest.mark.asyncio
    async def test_get_execution_id_increments(self, controller):
        # When
        id1 = await controller.get_execution_id()
        id2 = await controller.get_execution_id()
        id3 = await controller.get_execution_id()
    ...
    @pytest.mark.asyncio
    async def test_get_execution_id_thread_safe(self, controller):
        # Given
        async def get_id():
            return await controller.get_execution_id()
